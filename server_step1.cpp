...
int main(){  
    ...
    //Export ciphertext
    int fileNUM = 128; //Get from client
  ...
  //Save add mul lists
    ...
    //Fix X.509.v3
    // SHA256 sha;
    // std::vector<int> tempAH;
    // sha.update(reinterpret_cast<char*>(buf));
    // for(int i = 0;i < 8; i++)
    //   tempAH.push_back(sha.m_state[i]);
    // save_maping_graph(tempAH, "server_folder/client_X_tempAH", 8);
    // std::cout<< "Save temp A-H finished" << std::endl;
